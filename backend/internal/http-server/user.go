...
// @Accept  json
// @Produce  json
// @Param input body GetUsernameByIDRequest true "Get username by id"
// @Success 200 {object} Response
// @Failure 400,404 {object} Response
// @Failure 500 {object} Response
// @Failure default {object} Response
...
func (h *Handler) getUsernameById(log *slog.Logger) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		localUserContext, err := h.services.Users.GetUserByID(req.ID)
		if err != nil {
		...
		if (user == domain.User{}) {
			log.Error("failed to get user by id", slog.String("error", "User not found"))
			render.JSON(w, r, response.Error("Something went wrong"))
			return
		}
	...
	//return h.services.User.GetUserByID(id)
}
